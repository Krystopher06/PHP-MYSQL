...
class MKMParser
{
	...
	function update_carte ()
	{
		...
		foreach ($html->find('div.article-row]') as $element) // TODO : mettez le bon sélecteur dans la fonction find() (voir explications ci-dessus)
		{
		...
		if ($nb_element > 0)
		{
			...
			if ($url_image != null)
            {
                $this->database->update('cartes', ["url_image" => $url_image], ['id' => $this->card->id]); // TODO update de l'url de l'image
            }
		}
