        ...
        if (!empty($mailR) && (filter_var($mailR, FILTER_VALIDATE_EMAIL))) {
            ...
            if ($result == isset($mailValidR)) {
                ...
                try {
                    ...
                    $mailExist = $pdoConnect->prepare("SELECT * FROM `utilisateurs` WHERE Email = '$mailR'");
                    ...
                    if ($mailExist) {
                        ...
                        $_SESSION['user_mailReset'] = $recupMail;
                        $recupCode ="";
                        for ($i=0; $i < 20 ; $i++) { 
                        ...
                        var_dump($recupCode);
                        $recupInsert = $pdoConnect -> prepare('INSERT INTO pwdreset(pwdResetEmail,)');
